---
title: 18 個核心動詞/運作詞 (Operators)
tags:
  - English
  - Basic_English
  - Grammar
child_of:
  - "[[Basic_English_Index]]"
sibling_of:
  - "[[Basic_English_Phrasal_Verbs]]"
  - "[[Basic_English_Vocabulary_Structure]]"
---
# 18 個核心動詞/運作詞 (Operators)

在 C.K. Ogden 的 Basic English 系統中，**動詞（Verbs）**被大幅刪除。奧格登認為大多數英語動詞都是「名詞或方向與基本動作的混合體」。因此，他保留了 18 個最基礎、最具物理方向性與邏輯功能的字，並將其稱為 **Operators (運作詞 / 算子)**。

這 18 個運作詞就像數學中的 `+`、`-`、`×`、`÷`，本身只指示最基本的「狀態」、「移動方向」或「因果關係」，但當它們與名詞、介系詞結合時，便能組合出整部英語 90% 以上的動作表達。

---

## 📊 18 個核心運作詞清單與功能分類

奧格登將這 18 個運作詞分為三大類：

### 1. 物理動作與方向運作詞 (10 個)
這 10 個動詞代表了人類最基本的空間位移與物理操縱動作：
- **come** (來) / **go** (去) — 空間位移的基本方向
- **put** (放) / **take** (拿) — 物理位置的改變
- **give** (給) / **get** (得) — 所有權的轉移
- **make** (製造/使) — 創造與因果的產生
- **keep** (保持) / **let** (容許) — 狀態的維持與釋放
- **seem** (似乎) — 感官上的狀態呈現

### 2. 輔助與靜態/感知運作詞 (6 個)
這 6 個動詞用於建立語法關係、靜態持有、基本感知與溝通：
- **be** (是/存在) — 最核心的狀態與繫詞
- **have** (有) — 擁有與隸屬關係
- **do** (做) — 一般性的活動算子與助動詞
- **say** (說) — 基本的語意輸出與交流
- **see** (看) — 視覺感知的基礎
- **send** (發送) — 物理或資訊的遠程傳遞

### 3. 情態助動詞 (2 個)
這 2 個情態算子用於表達時間未來性與許可可能性：
- **will** (將要/意志)
- **may** (可以/可能)

---

## 💡 運作詞的「名詞化代換」公式

為了消除數千個複雜動詞，Basic English 採用了一個核心公式：
$$\text{運作詞 (Operator)} + \text{介系詞 (Preposition) / 名詞 (Noun)} \Longrightarrow \text{複雜動詞}$$

### 對照實例：
| 複雜動詞 | Basic English 代換公式 | 說明 |
|:---|:---|:---|
| **examine** (檢查) | `give a look to` 或 `look at` | 運作詞 `give` + 名詞 `a look` + 介系詞 `to` |
| **decide** (決定) | `make up one's mind` | 運作詞 `make` + 介系詞 `up` + 名詞 `one's mind` |
| **enter** (進入) | `go into` | 運作詞 `go` + 介系詞 `into` |
| **depart** (離開) | `go away` | 運作詞 `go` + 介系詞 `away` |
| **construct** (建造) | `make` / `put together` | 運作詞 `make` 或 `put` + `together` |
| **understand** (理解) | `get the meaning of` | 運作詞 `get` + 名詞 `the meaning` + 介系詞 `of` |
| **disembark** (下船/飛機) | `get off` | 運作詞 `get` + 介系詞 `off` |

---

## 🔗 關係與連結

- **屬於本專案主頁之子頁面**：[[Basic_English_Index|基礎英語總覽 @child_of]]
- **其他子概念頁面**：
  - [[Basic_English_Phrasal_Verbs|動詞片語的代換組合 @sibling_of]] (進一步學習如何將 18 個 Operators 與介系詞組合)
  - [[Basic_English_Vocabulary_Structure|850單字分類結構 @sibling_of]] (了解 18 個 Operators 在整個詞彙表中的佔比)

---

## 🎯 FSI Pattern Drill

> [!tip] 基礎句
> We will **make up our mind** tomorrow.
> 我們明天會做出決定。

### 1️⃣ Substitution Drill（替換練習）

| Prompt | Expected Response |
|---|---|
| next week | We will **make up our mind** next week. |
| give a look to the book | We will **give a look to the book** tomorrow. |
| They | **They** will make up their mind tomorrow. |
| may | We **may** make up our mind tomorrow. |

### 2️⃣ Transformation Drill（轉換練習）

| Transformation | Expected Response |
|---|---|
| → Negative | We **will not** make up our mind tomorrow. |
| → Question | **Will we** make up our mind tomorrow? |
| → Past Tense | We **made up** our mind yesterday. |
| → Use "decide" | We will **decide** tomorrow. |

### 3️⃣ Response Drill（應答練習）

| Prompt | Expected Response |
|---|---|
| Will you make up your mind tomorrow? | Yes, we will make up our mind tomorrow. |
| When will they make up their mind? | They will make up their mind next week. |
| Did he make up his mind yesterday? | No, he did not make up his mind yesterday. |
| Who will make up their mind tomorrow? | The managers will make up their mind tomorrow. |

### 4️⃣ Expansion Drill（擴展練習）

| Step | Sentence |
|---|---|
| 核心 | Make up your mind. |
| +subject | We make up our mind. |
| +helper | We will make up our mind. |
| +time | We will make up our mind tomorrow. |
| +condition | We will make up our mind tomorrow **if we get the information**. |

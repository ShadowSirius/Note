---
part_of:
  - "[[Japanese_Grammar_Index]]"
contradicts:
  - "[[JP_Grammar_43_te_agemasu]]"
replaces:
  - "[[JP_Grammar_44_te_moraimasu]]"
---
# 〜てくれます

## 規則與接續
- `（施惠者）が + （我/我方人員）に + 動詞て形 + くれます`

## 用法說明
表示「他人主動為我（或我方親友）做某個有利的動作」，語氣中隱含著說話者的感激之情。
- **主詞**：必須是「提供好意的他人」。
- **接受者**：是第一人稱的「我」或我方人員（通常可省略 `私に`）。

## 實用例句
> [!tip] 句子結構色彩：🟢主語/主題 \| 🔵核心/謂語 \| 🟠修飾語 \| 🔴時間/場所/副詞

- 日文：<span style="color:#2ECC71"><ruby>友達<rt>ともだち (tomodachi)</rt></ruby>が</span><span style="color:#3498DB">レポートを<ruby>見<rt>み (mi)</rt></ruby>てくれました。</span>
- 中文：朋友幫我看了報告。（感激朋友）
- 日文：<span style="color:#2ECC71"><ruby>先輩<rt>せんぱい (senpai)</rt></ruby>は</span><span style="color:#3498DB"><ruby>質問<rt>しつもん (shitsumon)</rt></ruby>に</span><span style="color:#FF477E"><ruby>丁寧<rt>ていねい (teinei)</rt></ruby>に</span><span style="color:#3498DB"><ruby>答<rt>こた (kota)</rt></ruby>えてくれます。</span>
- 中文：前輩總是耐心地回答我的問題。
- 日文：<span style="color:#FF477E"><ruby>明日<rt>あした (ashita)</rt></ruby></span><span style="color:#FF477E"><ruby>駅<rt>えき (eki)</rt></ruby>まで</span><span style="color:#FF477E"><ruby>車<rt>くるま (kuruma)</rt></ruby>で</span><span style="color:#3498DB"><ruby>送<rt>おく (oku)</rt></ruby>ってくれます。</span>
- 中文：他明天會開車送我到車站。
- 日文：<span style="color:#FF477E"><ruby>忙<rt>いそが (isoga)</rt></ruby>しいのに</span><span style="color:#3498DB"><ruby>時間<rt>じかん (jikan)</rt></ruby>を<ruby>作<rt>つく (tsuku)</rt></ruby>ってくれて</span><span style="color:#3498DB"><ruby>助<rt>たす (tasu)</rt></ruby>かりました。</span>
- 中文：謝謝你這麼忙還特地抽空，真是幫了大忙。

## 關係說明
- 與自己為他人提供好意的 [[JP_Grammar_43_te_agemasu|〜てあげます @contradicts]] 互為對稱（授受方向相反）。
- 與同表接受好意的 [[JP_Grammar_44_te_moraimasu|〜てもらいます @replaces]] 互為近義替換，但**主詞視角不同**（`てくれます` 以施惠者為主語；`てもらいます` 以受惠的「我」為主語，表示承蒙他人做事）。

---

## 🎯 FSI Pattern Drill

> [!tip] Base Sentence
> 友達（ともだち）がレポートを見て（みて）くれました。
> （朋友幫我看了報告。）

### 1️⃣ Substitution Drill（替換練習）

| Prompt | Expected Response | Translation（中文翻譯） |
|---|---|---|
| 先輩（せんぱい） | **先輩（せんぱい）**がレポートを見て（みて）くれました。 | 前輩幫我看了報告。 |
| 先生（せんせい） | **先生（せんせい）**がレポートを見て（みて）くれました。 | 老師幫我看了報告。 |
| 彼女（かのじょ） | **彼女（かのじょ）**がレポートを見て（みて）くれました。 | 女朋友幫我看了報告。 |
| お父さん（おとうさん） | **お父さん（おとうさん）**がレポートを見て（みて）くれました。 | 父親幫我看了報告。 |

### 2️⃣ Transformation Drill（轉換練習）

| Transformation | Expected Response | Translation（中文翻譯） |
|---|---|---|
| 非過去形（ひかこけい） | 友達（ともだち）がレポートを見て（みて）**くれます**。 | 朋友會幫我看看報告。 |
| 普通形過去（ふつうけいかこ） | 友達（ともだち）がレポートを見て（みて）**くれた**。 | 朋友幫我看了報告。（常體） |
| 否定形過去（ひていけいかこ） | 友達（ともだち）がレポートを見て（みて）**くれませんでした**。 | 朋友沒有幫我看看報告。 |
| 疑問形（ぎもんけい） | 友達（ともだち）がレポートを見て（みて）くれました**か**。 | 朋友幫你看了報告嗎？ |

### 3️⃣ Response Drill（應答練習）

| Prompt | Expected Response | Translation（中文翻譯） |
|---|---|---|
| 誰（だれ）がレポートを見て（みて）くれましたか。 | 友達（ともだち）が見て（みて）くれました。 | 誰幫你看了報告？ / 朋友幫我看了。 |
| 先輩（せんぱい）は丁寧（ていねい）に答（こた）えてくれますか。 | はい、丁寧（ていねい）に答（こた）えてくれます。 | 前輩會耐心地回答嗎？ / 是的，會耐心地回答。 |
| 明日（あした）駅（えき）まで送（おく）ってくれますか。 | はい、送（おく）ってくれます。 | 明天會送我到車站嗎？ / 是的，會送我到車站。 |

### 4️⃣ Expansion Drill（擴展練習）

| Step | Sentence | Translation（中文翻譯） |
|---|---|---|
| 核心（かくしん） | 見て（みて）くれました。 | 幫我看了。 |
| +目的語（もくてきご） | レポートを見て（みて）くれました。 | 幫我看了報告。 |
| +主語（しゅご） | 友達（ともだち）がレポートを見て（みて）くれました。 | 朋友幫我看了報告。 |
| +恩恵（おんけい） | 私（わたし）のために友達（ともだち）がレポートを見て（みて）くれました。 | 朋友特地為我看了報告。 |
